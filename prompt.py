...
import os
import cv2
videos = os.listdir("original_moca")

def get_prompt(video):
    video_path = os.path.join("original_moca", video)
    image_path = os.path.join(video_path)
    imgs = [Image.open(image_path)]
    # Capture image and get prompt
    prompt = caption_image(imgs, video) 
    
    with open(os.path.join("./prompts", video.replace(".jpg","")), "w") as f:
        f.write(prompt)
    print(prompt)
    return prompt
